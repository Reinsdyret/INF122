...
zeroCrossing :: (Num a, Ord a) => a -> a -> Bool
zeroCrossing a b = a > 0 && b <= 0 || b > 0 && a <= 0
...
zeroCrossings [] = 0
zeroCrossings list
    | zeroCrossing (head list) (head (drop 1 list)) = 1 + zeroCrossings (drop 2 list)
    | otherwise = zeroCrossings (drop 2 list)
